...
def user_input():
    ...
    if object_check["config_file"] != "":
        ...
        parser.add_argument(
            "--download", default=False, help="Download images", action="store_true"
        )
        parser.add_argument(
            "-iu",
            "--ignore_urls",
        ...
        parser.add_argument(
            "--search", type=str, default="", help="search string, i.e. bees on flowers"
        )
...
class googleimagesdownload:
    ...
    def download_extended_page(self, url, chromedriver):
        ...
        options.binary_location = "/Users/hannojacobs/chrome/mac_arm-114.0.5735.90/chrome-mac-arm64/Google Chrome for Testing.app/Contents/MacOS/Google Chrome for Testing"
        ...
        try:
            ...
            print(
                ...
                "or google chrome browser is not installed on your machine (exception: %s)"
                % e
            )
            sys.exit()
        ...
        pbar = tqdm(
            enumerate(range(30)), desc="Downloading HTML...", total=30
        )  # progress bar
        for _ in pbar:
            try:  # click 'see more' button if found
                # browser.find_element_by_id("smb").click()  # google images 'see more' button
                browser.find_element(
                    By.CLASS_NAME, "btn_seemore"
                ).click()  # bing images 'see more' button
            except:
                pass
    ...
    def _get_next_item(self, s):
        ...
        if start_line == -1:  # If no links are found then give an error!
            ...
            try:
                object_decode = bytes(html.unescape(object_raw), "utf-8").decode(
                    "unicode_escape"
                )
